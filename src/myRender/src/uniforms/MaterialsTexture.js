  ...
  updateFrom(materials, textures) {
    ...
    function writeTextureMatrixToArray(material, textureKey, array, offset) {
      ...
      if (threeCompatibilityTransforms) {
        ...
        texture =
          material[textureKey] && material[textureKey].isTexture
            ? material[textureKey]
            : null;
      }
    ...
    for (let i = 0, l = materials.length; i < l; i++) {
      ...
      const iridescenceThicknessRange = getField(
        m,
      ...
      // isThinFilm
      const isThinFilm =
        getField(m, 'thickness', 0.0) === 0.0 &&
        getField(m, 'attenuationDistance', Infinity) === Infinity;
      ...
      index++; // shadow
      floatArray[index++] =
        Number(m.vertexColors) | (Number(m.flatShading) << 1); // vertexColors & flatShading
      ...
      index += writeTextureMatrixToArray(
        m,
        'transmissionMap',
        floatArray,
        index
