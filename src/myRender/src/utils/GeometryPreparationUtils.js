...
export function getGroupMaterialIndicesAttribute(
  geometry,
  materials,
  allMaterials
) {
...
export function setCommonAttributes(geometry, options) {
  ...
  if (!geometry.attributes.uv && attributes && attributes.includes('uv')) {
    ...
    geometry.setAttribute(
      'uv',
      new BufferAttribute(new Float32Array(vertCount * 2), 2, false)
    );
  }
  ...
  if (
    ...
    if (normalMapRequired) {
      ...
      geometry.setAttribute(
        'tangent',
        new BufferAttribute(new Float32Array(vertCount * 4), 4, false)
      );
    }
  }
...
export function mergeMeshes(meshes, options = {}) {
  ...
  for (let i = 0, l = meshes.length; i < l; i++) {
    ...
    const geometry = options.cloneGeometry
    ...
    const materialIndexAttribute = getGroupMaterialIndicesAttribute(
      geometry,
      mesh.material,
      materials
    );
